...
# 1. Wczytaj model
model = joblib.load('fifa_value_predictor.pkl')
...
results = pd.DataFrame({
    ...
    'Ocena': new_data['overall'],
    'Wiek': new_data['age'],
    'Przewidywana wartość (€)': [f"€{x:,.2f}" for x in predictions],
    'Wartość (mln €)': [f"€{x/1_000_000:,.2f} mln" for x in predictions]
})

print("\nPRZEWIDYWANE WARTOŚCI RYNKOWE ZAWODNIKÓW")
...
real_players = pd.DataFrame({
    ...
    'real_value_eur': [78000000, 119500000, 45000000]
})

# 3. Przygotowanie danych do predykcji
prediction_data = real_players.drop(['short_name', 'real_value_eur'], axis=1)
...
results = pd.DataFrame({
    ...
    'Ocena': real_players['overall'],
    'Wiek': real_players['age'],
    'Rzeczywista wartość (€)': [f"€{x:,.2f}" for x in real_players['real_value_eur']],
    'Przewidywana wartość (€)': [f"€{x:,.2f}" for x in predictions],
    'Różnica (%)': [f"{(predictions[i] - real_players['real_value_eur'][i])/real_players['real_value_eur'][i]*100:.1f}%" 
